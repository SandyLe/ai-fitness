$(document).ready(function() {
  // 获取用户ID
  const clicked = $('#clickedBtn').val();
  if (clicked > 0) {
    $('.status-value').html('已打卡')
    $('.status-value').css('color', '#4caf50');
    $('.status-value').css('background-color', '#e8f5e9');

    $('.clock-btn').text('今日已打卡');
    $('.clock-btn').prop("disabled", true);
    $('.clock-btn').css('background', 'linear-gradient(to right, #9e9e9e, #bdbdbd)');
    $('.clock-btn').css('cursor', 'default');
  }
