...
TABLE_NAME = "course_action_comment"


def add_action_comment(action_comment: dict):
    """增加用户动作结果评价"""
    if not action_comment or len(action_comment) == 0:
    ...
    for field in required_fields:
        if field not in action_comment or not action_comment.get(field):
             if action_comment.get(field) is None:
                 return Response.fail(code=500, msg=f"字段 {field} 不能为空")

    # 准备插入的数据
    insert_data = action_comment.copy()
...
# def update_action_comment(record_id: int, update_data: dict): ...

def delete_action_comment_by_id(record_id: int):
    """根据ID删除动作结果评价 (物理删除)"""
    if not record_id:
...
def get_action_comment(condition: dict):
    """获取动作结果评价，允许传入查询条件字典"""
...
def get_action_comment_by_id(record_id: int):
     """根据ID获取单个动作结果评价"""
     if not record_id:
         return Response.fail(code=500, msg="动作结果评价ID不能为空")
     return get_action_comment({"id": record_id})
