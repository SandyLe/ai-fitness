...
TABLE_NAME = "user_plan_detail"


def add_plan_detail(plan_detail_data: dict):
    """增加用户计划"""
    if not plan_detail_data or len(plan_detail_data) == 0:
        return Response.fail(code=500, msg="提交信息为空")

    # 必填字段校验
    required_fields = ['user_id', 'plan']
    for field in required_fields:
        if field not in plan_detail_data or not plan_detail_data.get(field):
             # Check for 0 explicitly if needed
             if plan_detail_data.get(field) is None:
                 return Response.fail(code=500, msg=f"字段 {field} 不能为空")

    # 准备插入的数据
    insert_data = plan_detail_data.copy()
    now = datetime.now()
...
def update_plan_detail(plan_id: int, update_data: dict):
...
def delete_plan_detail_by_id(plan_detail_id: int):
    """根据ID删除用户计划 (逻辑删除)"""
...
def get_plan_detail(condition: dict):
...
def get_plan_detail_by_id(plan_detail_id: int):
     """根据ID获取单个用户计划信息"""
     if not plan_detail_id:
         return Response.fail(code=500, msg="计划ID不能为空")
     return get_plan_detail({"id": plan_detail_id})
